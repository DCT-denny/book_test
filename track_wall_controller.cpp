...
void funcase_controllers::TrackWallController::update(const ros::Time &time, const ros::Duration &period){
  ...
  */
  ...
  r_angle_arr[2] = _err_ran;
  r_angle_arr[3] = _err_ang;
  r_angle_arr[4] = sum;
  for(int i =0;i<5;i++){
   wall_fuzzy_arr[i]=fuzzyError_ran[i];
  }
  for(int i =5;i<10;i++){
   wall_fuzzy_arr[i]=fuzzyError_ang[i-5];
  }
  std_msgs::Float64MultiArray r_angle_err,wall_fuzzy;
  
  for(int i =0;i<5;i++){
   r_angle_err.data.pushback(r_angle_arr[i]);
  }
  for(int i =0;i<10;i++){
   wall_fuzzy.data.pushback(wall_fuzzy_arr[i]);
  }
